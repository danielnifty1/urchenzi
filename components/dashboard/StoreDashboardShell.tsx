...
export function StoreDashboardShell({ children }: { children: React.ReactNode }) {
  ...
  const workspaceTabs = useMemo(
    () => (isStoreManager ? TABS.filter((t) => t.path !== "/team") : TABS),
    [isStoreManager],
  );
    ...
    <div className="flex min-h-screen flex-col bg-[#0f1419] text-white">
      ...
      >
        <div className="flex h-16 items-center gap-2 border-b border-white/10 px-5">
          ...
          <div className="min-w-0">
            <Link href="/dashboard" className="block text-sm font-bold tracking-tight hover:text-emerald-300">
              {isStoreManager ? "Dashboard" : "Vendor dashboard"}
            </Link>
        ...
        <nav className="space-y-1 p-3">
          ...
          <Link
            href={isStoreManager ? href(storeId, "/finance") : "/dashboard/finance"}
            onClick={() => setMobileOpen(false)}
            className={clsx(
              "mb-3 block rounded-xl px-3 py-2 text-xs font-medium transition",
              pathname.startsWith("/dashboard/finance") || pathname.startsWith(href(storeId, "/finance"))
